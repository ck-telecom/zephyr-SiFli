...
static int i2c_sf32lb_configure(const struct device *dev, uint32_t dev_config)
{
	...
	case I2C_SPEED_STANDARD:
		data->hi2c.Init.ClockSpeed = 100000U;
		break;

	case I2C_SPEED_FAST:
		data->hi2c.Init.ClockSpeed = 400000U;
		break;
	...
	if (dev_config & I2C_ADDR_10_BITS) {
		data->hi2c.Init.AddressingMode = I2C_ADDRESSINGMODE_10BIT;
	} else {
	...
	// data->handle.core = bf0_i2c->bf0_i2c_cfg->core;
	data->hi2c.Mode = HAL_I2C_MODE_MASTER;
...
static int i2c_sf32lb_init(const struct device *dev)
{
	...
	err = i2c_sf32lb_configure(dev, I2C_MODE_CONTROLLER | i2c_map_dt_bitrate(cfg->bitrate));
	if (err < 0) {
		return err;
	}
